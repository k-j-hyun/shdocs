#!/usr/bin/env python3
"""
Generate PWA icons from SVG
"""

import os
from pathlib import Path

# SVG icon content
SVG_CONTENT = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  <defs>
    <linearGradient id="bg" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#FF6B9D;stop-opacity:1" />
      <stop offset="100%" style="stop-color:#FFB6C1;stop-opacity:1" />
    </linearGradient>
  </defs>
  
  <!-- Background circle -->
  <circle cx="256" cy="256" r="240" fill="url(#bg)" stroke="#fff" stroke-width="8"/>
  
  <!-- Calendar icon -->
  <rect x="128" y="160" width="256" height="192" rx="16" ry="16" fill="#fff" opacity="0.95"/>
  
  <!-- Calendar header -->
  <rect x="128" y="160" width="256" height="48" rx="16" ry="16" fill="#fff"/>
  <rect x="128" y="192" width="256" height="16" fill="#FF6B9D"/>
  
  <!-- Calendar rings -->
  <rect x="176" y="144" width="8" height="32" rx="4" ry="4" fill="#666"/>
  <rect x="328" y="144" width="8" height="32" rx="4" ry="4" fill="#666"/>
  
  <!-- Calendar grid -->
  <line x1="160" y1="240" x2="352" y2="240" stroke="#eee" stroke-width="1"/>
  <line x1="160" y1="272" x2="352" y2="272" stroke="#eee" stroke-width="1"/>
  <line x1="160" y1="304" x2="352" y2="304" stroke="#eee" stroke-width="1"/>
  
  <line x1="192" y1="208" x2="192" y2="336" stroke="#eee" stroke-width="1"/>
  <line x1="224" y1="208" x2="224" y2="336" stroke="#eee" stroke-width="1"/>
  <line x1="256" y1="208" x2="256" y2="336" stroke="#eee" stroke-width="1"/>
  <line x1="288" y1="208" x2="288" y2="336" stroke="#eee" stroke-width="1"/>
  <line x1="320" y1="208" x2="320" y2="336" stroke="#eee" stroke-width="1"/>
  
  <!-- Some calendar events -->
  <circle cx="176" cy="224" r="4" fill="#FF6B9D"/>
  <circle cx="240" cy="256" r="4" fill="#4ECDC4"/>
  <circle cx="304" cy="288" r="4" fill="#45B7D1"/>
  
  <!-- Sheets icon overlay -->
  <rect x="320" y="280" width="48" height="36" rx="4" ry="4" fill="#34A853" opacity="0.9"/>
  <rect x="324" y="284" width="40" height="4" fill="#fff" opacity="0.8"/>
  <rect x="324" y="292" width="40" height="4" fill="#fff" opacity="0.8"/>
  <rect x="324" y="300" width="40" height="4" fill="#fff" opacity="0.8"/>
  <rect x="324" y="308" width="40" height="4" fill="#fff" opacity="0.8"/>
</svg>'''

def generate_png_from_svg(svg_content, output_path, size):
    """Generate PNG from SVG using basic HTML/CSS (for demonstration)"""
    # For now, we'll create a simple HTML file that can be screenshot
    # In a real implementation, you'd use libraries like cairosvg or Pillow
    
    html_content = f'''<!DOCTYPE html>
<html>
<head>
    <style>
        body {{ margin: 0; padding: 20px; background: white; }}
        .icon {{ width: {size}px; height: {size}px; }}
    </style>
</head>
<body>
    <div class="icon">{svg_content}</div>
</body>
</html>'''
    
    # Save HTML file for manual conversion
    html_path = output_path.replace('.png', '.html')
    with open(html_path, 'w') as f:
        f.write(html_content)
    
    # Create a placeholder file for now
    placeholder_content = f'''# Icon Placeholder
    
This is a placeholder for {size}x{size} icon.
To generate actual PNG files, you can:

1. Open the generated HTML file in a browser
2. Use browser developer tools to screenshot the icon
3. Or use a tool like:
   - cairosvg: pip install cairosvg
   - Pillow with svg support
   - Online SVG to PNG converters

HTML file created: {html_path}
'''
    
    with open(output_path.replace('.png', '_instructions.txt'), 'w') as f:
        f.write(placeholder_content)

def main():
    # Icon sizes for PWA
    sizes = [72, 96, 128, 144, 152, 192, 384, 512]
    
    icons_dir = Path('static/icons')
    icons_dir.mkdir(exist_ok=True)
    
    # Save SVG
    svg_path = icons_dir / 'icon.svg'
    with open(svg_path, 'w') as f:
        f.write(SVG_CONTENT)
    
    print(f"Generated SVG: {svg_path}")
    
    # Generate PNG placeholders and HTML files
    for size in sizes:
        png_path = icons_dir / f'icon-{size}x{size}.png'
        generate_png_from_svg(SVG_CONTENT, str(png_path), size)
        print(f"Generated placeholder for: {png_path}")
    
    print("\nTo generate actual PNG files:")
    print("1. Install cairosvg: pip install cairosvg")
    print("2. Run: python -c 'import cairosvg; [cairosvg.svg2png(url=\"static/icons/icon.svg\", write_to=f\"static/icons/icon-{s}x{s}.png\", output_width={s}, output_height={s}) for s in [72,96,128,144,152,192,384,512]]'")
    
if __name__ == '__main__':
    main()